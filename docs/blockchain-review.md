...
- 이 브랜치의 현재 변경: **검토 문서와 재현 테스트만 추가. 실제 동작을 수정하지 않음.**

기본 등록·승인·폐기 동작은 구현되어 있다. 먼저 조회 예외와 배포 설정을 정리하고, 공개 Registry의 식별자·폐기 정책을 확정한 뒤 독립적인 운영 CLI를 추가하는 순서를 권장한다.
...
| R3  | 재현된 예외 처리 오류      | 미등록 Tool의 정상적인 `ToolNotFound`가 Registry 장애로 표시됨                                  | 즉시 수정                    |
| R4  | 설정 검증 부족             | 배포 클라이언트는 기대 `CHAIN_ID`를 확인하지 않고, API의 배포 파일 로더는 파일의 chainId를 무시 | 외부 테스트넷 배포 전 높음   |
...
## R3. 미등록 Tool이 Registry 장애로 처리됨

위치: [registry.ts](../apps/api/src/registry.ts) 44–71행, [gateway.ts](../apps/api/src/gateway.ts) 171–176행.

`Registry.get()` 인터페이스는 `RegistryRecord | null`을 반환하도록 되어 있다. 데모 어댑터는 미등록 시 `null`을 반환하지만, 온체인 어댑터는 컨트랙트의 `ToolNotFound`를 그대로 던진다. 게이트웨이는 이 예외를 네트워크 장애와 똑같이 처리한다.
...
## R4. 배포와 조회의 네트워크 설정 불일치

위치: [clients.ts](../packages/contracts/scripts/clients.ts) 21–58행, [registry.ts](../apps/api/src/registry.ts) 74–79행, [API 시작 설정](../apps/api/src/index.ts) 29–47행.
...
1. **R3:** 미등록과 조회 장애를 구분. 변경 범위가 작고 API 담당자와 바로 통합 가능.
2. **R4:** 공통 네트워크 설정과 배포 전 검증. 테스트넷 실수 방지.
...
재현 코드는 `packages/contracts/review/findings.test.ts`에 있다. 일반 `test/`와 구분하여 명시적으로 실행한다. 독립된 메모리 테스트 체인과 임시 loopback RPC를 사용하므로 기존 로컬 체인이나 테스트넷의 상태를 바꾸지 않는다.

현재 문제 동작을 확인하는 **재현 테스트 5개**다. 이 테스트가 통과했다는 것은 문제가 재현됐다는 뜻이지, 문제가 수정됐다는 뜻이 아니다. 수정할 때 해당 테스트를 원하는 정상 동작을 검증하는 회귀 테스트로 바꿔 `test/` 또는 API 테스트에 옮긴다.
